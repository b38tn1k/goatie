...
- What acts on a thing lives on the thing: labels above their fields, errors beside the field they name, confirm where the click just was. Intent survives everything — redirects return to the original destination, validation never wipes fields, back restores scroll and filters. Pages compose by use: daily work above one-time setup, independent panels side-by-side when width allows — never a phone column on a desktop screen.
